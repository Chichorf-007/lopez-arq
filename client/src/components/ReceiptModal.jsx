...
export default function ReceiptModal({ settlement, onClose }) {
  ...
  const { user_name, user_username, rate_per_hour, total_hours, total_cost, projects, period_start, period_end } = settlement;
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div 
        ...
        style={{ maxWidth: '800px', width: '100%', padding: '2rem', background: '#FFF', color: '#000' }}
      >
        ...
        <div className="no-print" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid #eee', paddingBottom: '1rem' }}>
          <h3 style={{ color: '#0F172A', margin: 0 }}>🧾 Recibo de Pago de Honorarios</h3>
        ...
        <div id="printable-receipt" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", padding: '1rem' }}>
          ...
          <div style={{ textAlign: 'center', margin: '1.25rem 0' }}>
            <h2 style={{ fontSize: '1.25rem', color: '#0F172A', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 800, margin: 0 }}>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <h4 style={{ fontSize: '0.85rem', textTransform: 'uppercase', color: '#475569', marginBottom: '0.5rem', fontWeight: 700 }}>
              Detalle de Horas y Proyectos Trabados en la Semana:
            </h4>
                ...
                {projects.length === 0 ? (
                  ...
                  projects.map((p, idx) => (
                    <tr key={idx} style={{ borderBottom: '1px solid #E2E8F0', background: idx % 2 === 0 ? '#FFF' : '#F8FAFC' }}>
                      <td style={{ padding: '0.75rem 0.85rem', fontWeight: 600, color: '#0F172A' }}>{p.project_name}</td>
                      <td style={{ padding: '0.75rem 0.85rem', textAlign: 'center', fontWeight: 700, color: '#2563EB' }}>{p.hours.toFixed(1)} hs</td>
                      <td style={{ padding: '0.75rem 0.85rem', textAlign: 'right', color: '#475569' }}>{formatPYG(rate_per_hour)}</td>
                      <td style={{ padding: '0.75rem 0.85rem', textAlign: 'right', fontWeight: 700, color: '#0F172A' }}>{formatPYG(p.hours * rate_per_hour)}</td>
                    </tr>
                  ))
                )}
              </tbody>
          ...
          {/* Grand Total Card */}
          <div style={{ background: '#0F172A', color: '#FFF', borderRadius: '8px', padding: '1.25rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem' }}>
            <div>
              <span style={{ fontSize: '0.8rem', color: '#94A3B8', textTransform: 'uppercase', fontWeight: 700, letterSpacing: '0.05em', display: 'block' }}>
                TOTAL HORAS TRABAJADAS: {total_hours.toFixed(1)} HS
              </span>
              <span style={{ fontSize: '1rem', fontWeight: 600, color: '#F8FAFC' }}>
                MONTO TOTAL APROBADO A PAGAR:
              </span>
            </div>
            <div style={{ fontSize: '1.85rem', fontWeight: 800, color: '#EAB308', fontFamily: "'Outfit', sans-serif" }}>
              {formatPYG(total_cost)}
            </div>
          </div>
